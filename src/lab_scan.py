...
def main():
    ...
    # Define directories to scan for labs
    lab_directories = ["/gpfs/labs/"]  # Adjust as necessary
    skip_hidden = True

    # Create an output folder with the current date
    date_str = datetime.now().strftime('%Y%m%d')
    output_folder = f"/data/scan_output/lab_{date_str}"
    ...
    with open(os.path.join(output_folder, 'users.csv'), 'w', newline='', errors='ignore') as user_csv, \
        ...
        # Scan lab directories and append results to the CSV files
